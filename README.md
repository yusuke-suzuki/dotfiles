...
Global mise configuration template. `install.sh` also appends the [official activation lines](https://mise.jdx.dev/dev-tools/shims.html) to the shell config unless already present, so agent-driven commands resolve mise-managed tools without a `mise exec` prefix.
