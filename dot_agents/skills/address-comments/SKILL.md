...
2. Verify every factual reference the draft makes — PR and issue numbers, commit ids, quoted text, numeric claims — against its source or the data it summarizes, and correct every mismatch. An unverified reference blocks the plan and the posting exactly as a textlint report does.
